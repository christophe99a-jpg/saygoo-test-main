...
const generateReferenceVente = async () => {
  ...
  const numero = await prochainNumero(
    `seq_vente_${year}`,
    () => maxExistant(prisma.venteVehicule, prefix)
  );
...
const generateDossierReference = async () => {
  ...
  const numeroDossier = await prochainNumero(
    `seq_dossier_${year}`,
    () => maxExistant(prisma.dossier, prefix)
  );
  return `${prefix}${String(numeroDossier).padStart(6, '0')}`;
};
